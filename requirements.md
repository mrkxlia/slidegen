...
スライド作成ロジックは **Agent Skills（オープン仕様）準拠のスキル＋プラグイン**（`skills/slidegen/`）
として同梱し、Claude Code / Agent Plugins 1.0 対応クライアントの両方から利用できる。
...
- **NFR-LIB-3** ビルド/実行は **uv 統一**（→ [ADR 0001](docs/adr/0001-uv-for-python-packaging.md)）。
...
- **壁打ち**: AI と対話して要件・構成を固める工程（`skills/slidegen/SKILL.md` に従う
  エージェントとの対話がこれを担う）。
