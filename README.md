...
スライド作成ロジックは **Agent Skills（オープン仕様）準拠のスキル＋プラグイン**として同梱し、
...
Python は **uv 統一**（[ADR 0001](docs/adr/0001-uv-for-python-packaging.md)）。
...
docs/       要件補助・仕様補助・ADR・設計・型カタログ・開発の経緯(history)
```
...
| [docs/history.md](docs/history.md) | 開発の経緯（過去の構成・移行の記録はすべてここ） |
...
```bash
uv run python -c "import slidegen, slidegen.render as r; print(len(r.RENDERERS))"   # → 168
```
...
一覧は [docs/type_catalog.md](docs/type_catalog.md)。
