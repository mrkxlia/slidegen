...
- **構成プレビュー**: 「👁 構成プレビュー」で DSL をパースし、各スライドを
  型・主張・要素のカードで一覧表示。**pptx の画素レンダリング（サムネイル画像）ではない**。
  真のサムネイルはブラウザだけでは生成できず（pptx→画像レンダラが必要）、無料CFでは
  サーバ側 LibreOffice 等が要るため非対応。構成確認はこのカードで代替する。
...
- **E2E(Playwright)**: `frontend/e2e/`（オプトイン）。gateway を route mock して UI フローを検証。
  導入・実行手順は `frontend/e2e/README.md`。
